...
# Como exemplo, modelares um carro elétrico. Esse carro é apenas um tipo específico, logo podemos basear-se na outra classe criada Car(), ou mais especificamente,
# Car4() do arquivo anterior.
class Car():
...
# Começamos colocar a nossa classe já feita na Parte 1 que foi Car(). Quando criamos a classe-filha, a classe-pai deve fazer parte do arquivo atual,
# logo se torna o parâmetro da classe-filha.
...
# É criado uma instância da classe ElectricCar() e armazenamos o tesla nele. A linha chama o método __init__() 
# definido em ElectricCar que diz ao Python para chamar o método __init__() definido na classe-pai.
...
# Depois que tiver um classe-filha que herde de uma classe-pai, é possível acrescentar qualquer atributo ou método novo necessário 
# para diferenciar a classe filha da pai.
...
# Para isso, é preciso definir um método na classe filha com o mesmo nome do método do pai. Python despreza o método da classe-pai e 
...
# Por exemplo, se continuarmos com detalhes em ElectricCar,  podemos parar e transferir esse atributos e métodos para uma classe diferente chamada Battery, 
# e assim usar uma instância de Battery como atributo da classe ElectricCar.
class Car():
    def __init__(self, make, model, year):
        self.make= make 
        self.model = model
...
tesla.battery.decribe_battery()
#Esse linha acima informa ao python para usar a instância tesla, encontrar o atributo battery e chamar o método describe_battery() 
# associado à instância de Battery do atributo.
...
# é provável que vamos querer transferir o get_range() para a classe ElectricCar(), e o método get_range continuaria verificando a 
# capacidade da bateria antes de determinar as distâncias,
# de modo alternativo, poderia ter uma associação entre get_range() e a bateria, mas passaríamos um parâmetro a ele, como car_model. 
# Assim o método get_range() informaria a distância
